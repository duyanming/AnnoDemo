...
namespace Anno.QueryServices.Platform
{
    public class PlatformQuery : IPlatformQuery
    {
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult Export_User()
        {
            Npoi.ExcelHelper eh = new Npoi.ExcelHelper(DateTime.Now.ToString("yyyyMMddhhmmss"));
            DataTable user = _db.Ado.GetDataTable(@"SELECT m.account as 员工编号,m.name as 员工姓名,m.position as 员工职位
            ,m.timespan as 最近登录,m.rdt as 注册时间 FROM sys_member m");
            //eh.DataTableToExcel(user, true);
            return new ActionResult(false, null, null, "为实现Npoi");
        }
        #endregion
        /// <summary>
